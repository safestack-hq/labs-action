...
if [ $1 = "trivy" ]
then
  cache_dir="${RUNNER_TEMP}"
  echo "The cache dir is $cache_dir"
  echo ">>> Trivy mode"
  echo "Trivy version"
  bash -c "set -e; set -o pipefail; trivy --version"
  echo ">>> Trivy scanning table output"
  bash -c "set -e; set -o pipefail; TRIVY_CACHE_DIR=. trivy config -f table -s HIGH,CRITICAL ."
  echo ">>> Trivy scanning json file output"
  bash -c "set -e; set -o pipefail; TRIVY_CACHE_DIR=. trivy config -f json -s HIGH,CRITICAL -o trivy.json ."
